...
function solve(){
  ...
  function arrange(){
    ...
    for(let i=0;i<document.querySelector(".js-size").value;i++)
      ...
      if(flag){
        info=`${String. fromCharCode(8592)} Rearrange the matrix so pivot ≠ 0`;
        ...
        if(i===j){
          for(let b=i+1;b<matrix.length;b++){
            ...
            if(!(matrix[b][j].equals(new Fraction(0,1)))){
              ...
              info=(pivot.numerator>0)?`${String. fromCharCode(8592)} R${i+2} = R${i+2} - (${pivot}) R${i+1}`:`${String. fromCharCode(8592)} R${i+2} = R${i+2} + (${pivot.multiply(new Fraction(-1))}) R${i+1}`;
              stepsMatrix+=printStep(matrix,info);
            }
          }
        }
  ...
  let flag=true;
    ...
    for(let i=document.querySelector(".js-size").value-1;i>=0;i--){
      ...
      info+=`= ${matrix[i][matrix[i].length-1]} ${String. fromCharCode(8594)} X${i+1} = ${variables[variables.length-1][0]}`
      stepsMatrix+=printStep(matrix,info);
      }
      function check2(){
        for(let i=0;i<matrix.length;i++){
      ...
      info+=` = ${matrix[i][matrix[i].length-1]} ${String. fromCharCode(8594)} X${i+1} = ${string2}`
      stepsMatrix+=printStep(matrix,info);
      }}
    }
  let string="";
